...
const tabs = [
  ...
  { href: "/app", icon: Scissors, label: "Serviços" },
...
export function BottomNav() {
  ...
  return (
    <div style={{ borderTop: "1px solid var(--border)", background: "white", padding: "10px 0 6px", display: "flex", justifyContent: "space-around", flexShrink: 0 }}>
      {tabs.map((tab, i) => {
        const isActive = i === 0 ? pathname === "/app" : pathname === tab.href;
        const Icon = tab.icon;
        return (
          <Link key={i} href={tab.href} style={{ background: "none", border: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 12px", textDecoration: "none", opacity: isActive ? 1 : 0.45, transition: "opacity 0.2s" }}>
